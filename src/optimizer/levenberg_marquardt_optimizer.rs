    ...
    fn optimize(
        ...
        while iter < opt_option.max_iteration {
            ...
            if let Some(lm_step) = linear_solver.solve_jtj(jtr_ref, &jtj_regularized) {
                ...
                let actual_residual_change = residuals.as_ref().unwrap().as_ref().squared_norm_l2()
                    - new_residuals.as_ref().squared_norm_l2();
                ...
                if rho > 0.0 {
                    ...
                    if (last_err - current_error).abs()
                        < opt_option.min_abs_error_decrease_threshold
                    {
                        break;
                    }
                    if (last_err - current_error).abs() / last_err
